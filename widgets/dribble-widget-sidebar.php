...
function Load_Dribbble_Widget_Sidebar() {
	register_widget( 'Widget_Dribbble_Shots_Sidebar' );
}
add_action( 'widgets_init', 'Load_Dribbble_Widget_Sidebar' );

/* Dribbble widget class. */
class Widget_Dribbble_Shots_Sidebar extends WP_Widget {

	/* Set up the widget's unique name, ID, class, description, and other options. */
	function __construct() {
		
		$widget_ops = array('classname' => 'widget-dribbble-sidebar secondary-sidebar-widge', 'description' => __('Display your Dribbble shots on your sidebar.', 'cinergy'));
		$control_options = array(
			'width' => 200,
			'height' => 350
		);
        parent::__construct('cinergy-dribbble-sidebar', __( '[Cinergy Dribbble Sidebar]', 'cinergy'), $widget_ops, $control_options);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( !empty( $items ) ) :
echo '<div class="sidebar-widget-content"><ul class="dribble-profile lists-rundown">';
			foreach ( $items as $item ) :
				...
				$image = $image_url[1];
					...
					echo '<li><a class="dribbble-link" href="' . esc_url( $link ) . '">';
						echo '<img src="' . esc_html( $image ) . '" alt="' . esc_attr( $title ) . '" width="85" height="85" />';
					echo '</a></li><!-- .dribbble-link -->';
				

			endforeach;
echo '</ul></div><!-- .dribbble-wrap -->';
		endif;
