...
// widget function
	class am_widget_recent_tweets extends WP_Widget {
		
		public function __construct() {
			parent::__construct(
				'am_widget_recent_tweets', // Base ID
				'[Cinergy] Recent Tweets', // Name
				array( 'description' => __( 'Display recent tweets', 'cinergy' ), ) // Args
			);
		}
			...
			public function widget($args, $instance) {
				...
				if ( ! empty( $title ) ){ echo $before_title . $title . $after_title; }
					...
					//check if cache needs update
						$am_twitter_plugin_last_cache_time = get_option('am_twitter_plugin_last_cache_time');
						$diff = time() - $am_twitter_plugin_last_cache_time;
						$crt = $instance['cachetime'] * 3600;
						
					 //	yes, it needs update			
						if($diff >= $crt || empty($am_twitter_plugin_last_cache_time)){
							
							if(!require_once(FW_DIR . '/libs/twitteroauth.php')){ 
							...
							//save tweets to wp option 		
								update_option('am_twitter_plugin_tweets',serialize($tweets_array));							
								update_option('am_twitter_plugin_last_cache_time',time());
					...
					$am_twitter_plugin_tweets = maybe_unserialize(get_option('am_twitter_plugin_tweets'));
					if(!empty($am_twitter_plugin_tweets)){
						print '<div class="afooter-widget-content footer-recent-posts-widget">';
							$fctr = '1';
							foreach($am_twitter_plugin_tweets as $tweet){					
								if(!empty($tweet['text'])){
									if(empty($tweet['status_id'])){ $tweet['status_id'] = ''; }
									if(empty($tweet['created_at'])){ $tweet['created_at'] = ''; }
								
									print '<article class="footer-recent-posts">
											<h2 class="footer-recent-posts-heading">
												<a target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.am_convert_links($tweet['text']).'</a></h2><time class="footer-recent-posts-time">'.am_relative_time($tweet['created_at']).'</time>';
		...
		//save widget settings 
			public function update($new_instance, $old_instance) {				
				...
				if($old_instance['username'] != $new_instance['username']){
					delete_option('am_twitter_plugin_last_cache_time');
				}
		...
		//widget settings form	
			public function form($instance) {
				...
				<p><label>Exclude replies:</label>
					...
					if (!function_exists('am_convert_links')) {
						function am_convert_links($status,$targetBlank=true,$linkMaxLen=250){
					...
					if (!function_exists('am_relative_time')) {
						function am_relative_time($a) {
...
// register	widget
$register_recent_tweets = function()
{
   return register_widget("am_widget_recent_tweets");
};

add_action('widgets_init', $register_recent_tweets);