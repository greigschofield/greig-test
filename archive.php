...
<div class="container">
    <div class="row">
        <main role="main" class="2cols-blog-content span9">
            <?php if (have_posts()) : ?>
                <?php if (is_day()) : ?>
                    <h2><?php _e('Archive:','cinergy') ?> <?php echo get_the_date('D M Y'); ?></h2>                          
                <?php elseif (is_month()) : ?>
                    <h2><?php _e('Archive:','cinergy') ?> <?php echo get_the_date('M Y'); ?></h2>    
                <?php elseif (is_year()) : ?>
                    <h2><?php _e('Archive:','cinergy') ?> <?php echo get_the_date('Y'); ?></h2>                              
                <?php else : ?>
                    <h2><?php _e('Archive','cinergy') ?></h2>    
                <?php endif;
