...
class Tax_Meta_Class {
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . __( 'Delete this image','cinergy') . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete','cinergy') . "' /></a>";
   ...
   * 
   * Modified from a function used by "Verve Meta Boxes" plugin ( //goo.gl/aw64H )
   *
   * @since 1.0
   * @access public 
   */
  ...
  /**
  * Ajax callback for deleting files.
  * Modified from a function used by "Verve Meta Boxes" plugin (//goo.gl/LzYSq)
  * @since 1.0
  * @access public
  */
  public function wp_ajax_delete_image() {
    ...
    if ( $ok ){
      ...
      echo json_encode(array('message' => __( 'Cannot delete file. Something\'s wrong.','cinergy')));
      die();
    }
  }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        if ($this->_Local_images){
          ...
          echo '//i.imgur.com/g8Duj.png';
        }
        echo '" alt="'.__('Remove','cinergy').'" title="'.__('Remove','cinergy').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    if ($this->_Local_images){
      ...
      echo '//i.imgur.com/w5Tuc.png';
    }
    echo '" alt="'.__('Add','cinergy').'" title="'.__('Add','cinergy').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( !empty( $meta )  && count($meta) > 0 && !$this->is_array_empty($meta) ) {
        ...
        echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files','cinergy') . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}||{$field['id']}|{$att}'>" . __( 'Delete','cinergy' ) . "</a>)</li>";
        }
        echo '</ol>';
      }
    ...
    echo "<div class='at-file-upload-label'> \n
      <strong>" . __( 'Upload new files','cinergy' ) . "</strong>\n
    </div>\n";
    echo "<div class='new-files'>\n
      <div class='file-input'>\n
        <input type='file' name='{$field['id']}[]' />\n
      </div><!-- End .file-input -->\n
      <a class='at-add-file button' href='#'>" . __( 'Add more files','cinergy' ) . "</a>\n
      </div><!-- End .new-files -->\n";
    echo "</td>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_color( $field, $meta ) {
    ...
    //  echo "<a href='#' class='at-color-select button' rel='{$field['id']}'>" . __( 'Select a color' ) . "</a>";
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color','cinergy' ) . "'/>";
   ...
   *    'std' => // default value, string optional
   *    'validate_func' => // validate function, string optional
   *    'format' => // date format, default yy-mm-dd. Optional. Default "'d MM, yy'"  See more formats here: //goo.gl/Wcwxn
   *  @param $repeater bool  is this a field inside a repeatr? true|false(default) 
   */
   ...
   *    'std' => // default value, string optional
   *    'validate_func' => // validate function, string optional
   *    'format' => // time format, default hh:mm. Optional. See more formats here: //goo.gl/83woX
   *  @param $repeater bool  is this a field inside a repeatr? true|false(default) 
   */
   ...
   *    'args' =>  // arguments to query taxonomy, see //goo.gl/uAANN default ('hide_empty' => false)  
   *  @param $args mixed|array
   *    'name' => // field name/label string optional
   *    'desc' => // field description, string optional
   *    'std' => // default value, string optional
   ...
   *    'args' =>  // arguments to query posts, see //goo.gl/is0yK default ('posts_per_page' => -1)  
   *  @param $args mixed|array
   *    'name' => // field name/label string optional
   *    'desc' => // field description, string optional
   *    'std' => // default value, string optional
